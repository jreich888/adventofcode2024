...
const NUM_WALL:i32 = -1;
const NUM_SPACE:i32 = std::i32::MAX;
...
fn print_num_map(nummap: &Vec<Vec<i32>>) {
    for r in nummap.clone() {
        for n in r.clone() {
            if n == -1 { print!("### ");} 
            else if n == std::i32::MAX { print!(" .  ");} 
            else { print!("{:3} ", n); }
        }
        println!("");
    }
...
fn flood_fill(map: &mut Vec<Vec<i32>>, start_pos: &geo::Coord<i32>) {
    let dirs = vec![ 
        coord!{x:-1,y:0},
        coord!{x:1,y:0},
        coord!{x:0,y:-1},
    ...
    // let mut cur = 1;
    map[start_pos.y as usize][start_pos.x as usize] = 0;
    let mut workqueue : VecDeque<geo::Coord<i32>> = VecDeque::new();
    workqueue.push_back(*start_pos);
...
fn calc_20ns_cheats(nummap : &mut Vec<Vec<i32>>, pos: &geo::Coord<i32>, cheat_count:&mut BTreeMap<i32,i32>) -> i32 {
    let pvalue = get_map(nummap, &pos);
    if pvalue == NUM_WALL { return 0; }

    let mut cheats_ge_50 = 0;

    // iterate over diamond pattern from here to any dest patterns within 20
    for x in 0..=20 {
        for y in 0..=(20-x) {
            if x+y > 20 { continue; }
            let dests = HashSet::from([ coord!{x:-x,y:-y},
                coord!{x:-x,y:y},
                coord!{x:x,y:-y},
                coord!{x:x,y:y} ]);
            ...
            for d in dests {
                let dest = *pos + d;
                let dvalue = get_map(nummap, &dest);
                if pvalue > dvalue { 
                    // reverse cheat? :)
                    continue;
                }
                if dvalue != NUM_WALL {
                    let cheat = (dvalue-pvalue).abs() - x - y;
                    if cheat >= 100 { 
                        cheats_ge_50 += 1; 
                        let prevcount = cheat_count.get(&cheat).unwrap_or(&0);
                        cheat_count.insert(cheat, prevcount+1);
                    }
                }
            }
        }
...
pub fn process_lines(lines:Vec<String>) -> u64 {
    ...
    for l in lines {
        let row : Vec<char> = l.trim().chars().collect();
        map.push(row.clone());
        // convert to numerical map
        let mut numrow : Vec<i32> = Vec::new();
        for c in row {
            numrow.push( 
                match c {
                    START => 0,
                    WALL => -1,
                    _ => std::i32::MAX // for both . and E
                }
            );
        }
        nummap.push(numrow);
    ...
    //     nummap.push(vec![std::i32::MAX; grid_size.x as usize]);
    // }
    ...
    //         nummap[c.y as usize][c.x as usize] = -1;
    //     }
    //     blist.push_back(c);
    //     count += 1;
    // }
    ...
    print_num_map(&mut nummap);


    flood_fill(&mut nummap, &start_pos);
    let end_count = get_map(&mut nummap, &end_pos);
    ...
    // calculate cheats 
    // no cheats on edges
    let mut cheat_count : BTreeMap <i32,i32> = BTreeMap ::new();
    let mut cheats_gt_100 = 0;
    let w = nummap[0].len();
    let h = nummap.len();
    let part1 = false;
    if part1 {
        for y in 1..h-1 {
            for x in 1..w-1 {
                ...
                if get_map(&mut nummap, &pos) == NUM_WALL {
                    let LEFT = coord!{x:-1,y:0};
                    let RIGHT = coord!{x:1,y:0};
                    let UP = coord!{x:0,y:-1};
                    let DOWN = coord!{x:0,y:1};
                    // look in 2 dirs left/right and up/down
                    let left = get_map(&mut nummap, &(pos+LEFT));
                    let right = get_map(&mut nummap, &(pos+RIGHT));
                    let up = get_map(&mut nummap, &(pos+UP));
                    let dn = get_map(&mut nummap, &(pos+DOWN));
                    if left != NUM_WALL && right != NUM_WALL {
                        let cheat = (left-right).abs()-2;
                        println!("Found horz cheat {cheat} at {:?}", pos);
                        if cheat >= 100 { cheats_gt_100 += 1; }
                        let prevcount = cheat_count.get(&cheat).unwrap_or(&0);
                        cheat_count.insert(cheat, prevcount+1);
                    }
                    if up != NUM_WALL && dn != NUM_WALL {
                        let cheat = (up-dn).abs()-2;
                        println!("Found vert cheat {cheat} at {:?}", pos);
                        if cheat >= 100 { cheats_gt_100 += 1; }
                        let prevcount = cheat_count.get(&cheat).unwrap_or(&0);
                        cheat_count.insert(cheat, prevcount+1);
                    }
    ...
    if true {
        for y in 1..h-1 {
            for x in 1..w-1 {
                let pos = coord!{x:x as i32,y:y as i32};
                let c_ge50 = calc_20ns_cheats(&mut nummap, &pos, &mut cheat_count);
                cheats_gt_100 += c_ge50;
            }
        }
    }

    for n in cheat_count.keys() {
        println!("cheat {n} count {}", cheat_count.get(n).unwrap());
    }
    println!("cheats over 100: {cheats_gt_100}");


    return cheats_gt_100 as u64;
    ...
    loop {
        ...
        // flood_fill(&mut nummap);
        let end_count = get_map(&mut nummap, 
            &coord!{x:grid_size.x-1,y:grid_size.y-1});
